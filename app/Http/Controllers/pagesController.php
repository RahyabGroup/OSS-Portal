<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;

use App\Http\Requests;

class pagesController extends Controller
{
    public function welcome()
	{
		return view('welcome');
	}
    public function forofor()
	{
		return view('404');
	}
    public function alwayMenu()
	{
		return view('alwayMenu');
	}
    public function alwayMenuIn()
	{
		return view('alwayMenuIn');
	}
    public function blankPage()
	{
		return view('blankPage');
	}
    public function calendar()
	{
		return view('calendar');
	}
    public function chartFlot()
	{
		return view('chartFlot');
	}
    public function chartMorris()
	{
		return view('chartMorris');
	}
    public function chartOther()
	{
		return view('chartOther');
	}
    public function CloseRightSlider()
	{
		return view('CloseRightSlider');
	}
    public function dashboard()
	{
		return view('dashboard');
	}
    public function dashboard2()
	{
		return view('dashboard2');
	}
    public function drop_upload()
	{
		return view('drop_upload');
	}
    public function fileManager()
	{
		return view('fileManager');
	}
    public function footerMenu()
	{
		return view('footerMenu');
	}
    public function footerShow()
	{
		return view('footerShow');
	}
    public function form()
	{
		return view('form');
	}
    public function form_ajax()
	{
		return view('form_ajax');
	}
    public function form_x_edit()
	{
		return view('form_x_edit');
	}
    public function formComponents()
	{
		return view('formComponents');
	}
    public function formMutiselect()
	{
		return view('formMutiselect');
	}
    public function formValidate()
	{
		return view('formValidate');
	}
    public function formWizard()
	{
		return view('formWizard');
	}
    public function hideUserPanel()
	{
		return view('hideUserPanel');
	}
    public function hideUserPanelIn()
	{
		return view('hideUserPanelIn');
	}
    public function images_manager()
	{
		return view('images_manager');
	}
    public function index()
	{
		return view('index');
	}
    public function load_img()
	{
		return view('load_img');
	}
    public function lockscreen()
	{
		return view('lockscreen');
	}
    public function login()
	{
		return view('login');
	}
    public function mailBox()
	{
		return view('mailBox');
	}
    public function map()
	{
		return view('map');
	}
    public function menu()
	{
		return view('menu');
	}
    public function menuReal()
	{
		return view('menuReal');
	}
    public function menuVertical()
	{
		return view('menuVertical');
	}
    public function page_invoice()
	{
		return view('page_invoice');
	}
    public function page_search()
	{
		return view('page_search');
	}
    public function pages_pricing()
	{
		return view('pages_pricing');
	}
    public function profile()
	{
		return view('profile');
	}
    public function register()
	{
		return view('register');
	}
    public function showhide()
	{
		return view('showhide');
	}
    public function showRightPanel()
	{
		return view('showRightPanel');
	}
    public function siteMap()
	{
		return view('siteMap');
	}
    public function slide()
	{
		return view('slide');
	}
    public function sliderIntro()
	{
		return view('sliderIntro');
	}
    public function table()
	{
		return view('table');
	}
    public function tableResponsive()
	{
		return view('tableResponsive');
	}
    public function tagInput()
	{
		return view('tagInput');
	}
    public function timeline()
	{
		return view('timeline');
	}
    public function topMenu()
	{
		return view('topMenu');
	}
    public function ui()
	{
		return view('ui');
	}
    public function ui_alert()
	{
		return view('ui_alert');
	}
    public function ui_button()
	{
		return view('ui_button');
	}
    public function ui_icon()
	{
		return view('ui_icon');
	}
    public function ui_modal()
	{
		return view('ui_modal');
	}
    public function ui_nestable()
	{
		return view('ui_nestable');
	}
    public function ui_panel()
	{
		return view('ui_panel');
	}
    public function ui_slide()
	{
		return view('ui_slide');
	}
    public function ui_typography()
	{
		return view('ui_typography');
	}
	public function switch_lang()
	{
		return view('switch_lang');
	}
    public function lang_en()
	{
		return view('lang_en');
	}
    public function lang_fa()
	{
		return view('lang_fa');
	}
	public function define_lang()
	{
		return view('define_lang');
	}
	public function loginn()
	{
		return view('loginn');
	}
	public function checklogin()
	{
		return view('checklogin');
	}
	public function generate()
	{
		return view('generate');
	}
	public function dashboardTwo()
	{
		return view('dashboard2');
	}
public function dashboardThree()
	{
		return view('dashboard3');
	}
public function dashboardFour()
	{
		return view('dashboard4');
	}
	public function dashboardFive()
	{
		return view('dashboard5');
	}
	public function dashboardSix()
	{
		return view('dashboard6');
	}
	public function dashboardSeven()
	{
		return view('dashboard7');
	}
	public function dashboardEight()
	{
		return view('dashboard8');
	}
	public function hello()
	{
		return view('hello');
	}
	public function dashboardTest()
	{
		return view('dashboardTest');
	}
	public function getMyTask()
	{
		return view('getMyTask');
	}
	public function pagechat()
	{
		return view('page_chat');
	}
	public function ui_paneltwo()
	{
		return view('ui_panel2');
	}
	
	public function dashboardNine()
	{
		return view('dashboard9');
	}
	
	public function pagination()
	{
		return view('pagination');
	}
	
	
	public function paginationx()
	{
		return view('paginationx');
	}
	
	
	public function ui_panelx()
	{
		return view('ui_panelx');
	}
	
	public function data()
	{
		return view('data');
	}
	
	
	public function dashboardTen()
	{
		return view('dashboard10');
	}
	
	public function paginationDetaile()
	{
		return view('paginationDetaile');
	}
	
	public function variables()
	{
		return view('variables');
	}
	
	public function taskDetailes()
	{
		return view('taskDetailes');
	}
	
}
